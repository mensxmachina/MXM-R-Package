IdentifyEquivalence.gee = function(queues, target, reps, group, dataset, test, wei, threshold, max_k, selectedVars, pvalues, stats, 
                                   remainingVars, univariateModels, selectedVarsOrder, hash, stat_hash, pvalue_hash, correl, se) { 
  ...
  for (cvar in varsToBeConsidered) {
    ...
    while (klimit <= k)  {
      ...
      for ( i in 1:ncol(subsetcsk) ) {
        z = subsetcsk[,i];
        z = t(t(z));
        cur_results = test(target, reps, group, dataset, cvar, z, wei = wei, univariateModels, hash = hash, stat_hash, pvalue_hash, correl = correl, se = se);
        ...
        if ( cur_results$pvalue > threshold ) {
          ...
          selectedVars[[cvar]] = 0;
          queues = identifyTheEquivalent.gee(queues, target, reps, group, dataset, cvar, z, test, wei, threshold, univariateModels, pvalues, hash, stat_hash, pvalue_hash, correl = correl, se = se);
          breakFlag = TRUE;
          break;
        }
      }
  ...
  results <- list(pvalues = pvalues , stats = stats , queues = queues , selectedVars = selectedVars , remainingVars = remainingVars, stat_hash = stat_hash, pvalue_hash = pvalue_hash, correl = correl, se = se);
  return(results);
}