IdentifyEquivalence.glmm = function(queues, target, reps, group, dataset, test, wei, threshold, max_k, selectedVars, pvalues, stats, remainingVars, univariateModels, selectedVarsOrder, hash, stat_hash, pvalue_hash, slopes)
{ 
    ...
    while (klimit <= k)  {
      ...
      for ( i in 1:ncol(subsetcsk) ) {
        ...
        if ( cur_results$pvalue > threshold ) {
          ...
          selectedVars[[cvar]] = 0;
          queues = identifyTheEquivalent.glmm(queues , target , reps, group, dataset , cvar , z , test , wei, threshold , univariateModels , pvalues, hash, stat_hash, pvalue_hash, slopes = slopes);
          breakFlag = TRUE;
          break;
        }
      }
