bic.llr.fsreg <- function( target, dataset, wei = NULL, tol = 0, ncores = 1) {
  ...
  durat <- proc.time()
  mi <- survival::survreg( target ~ 1, weights = wei, dist = "loglogistic") ## initial BIC
  la <- logLik(mi)
  ...
  if (ncores <= 1) {
    ...
    for (i in 1:p) {
      mi <- survival::survreg( target ~ dataset[, i], weights = wei, dist = "loglogistic")
      la <- logLik(mi)
    ...
    mod <- foreach( i = 1:p, .combine = rbind, .export = "survreg", .packages = "survival") %dopar% {
      ww <- survival::survreg( target ~ dataset[, i], weights = wei, dist = "loglogistic" )
      la <- logLik(ww)
  ...
  if ( ini - mat[sel, 2] > tol ) {
    ...
    mi <- survival::survreg( target ~ dataset[, sel], weights = wei, dist = "loglogistic" )
    la <- logLik(mi)
  ...
  if ( length(moda) > 0  &  nrow(mat) > 0 ) {
    ...
    if ( ncores <= 1 ) {
      bico <- numeric( pn )
      for ( i in 1:pn ) {
        ma <- survival::survreg( target ~., data = dataset[, c(sel, mat[i, 1]) ], weights = wei, dist = "loglogistic" )
        la <- logLik(mi)
        bico[i] <-  - 2 * as.numeric( la ) + attr(la, "df") * con
      }
      mat[, 2] <- bico
      ...
      mod <- foreach( i = 1:pn, .combine = rbind, .export = "survreg", .packages = "survival") %dopar% {
        ww <- survival::survreg( target ~ dataset[, sel ] + dataset[, mat[i, 1] ], weights = wei, dist = "loglogistic" )
        la <- logLik(ww)
        return( - 2 * as.numeric( la ) + attr(la, "df") * con )
      }
      stopCluster(cl)
  ...
  if ( nrow(info) > 1  &  nrow(mat) > 0 ) {
    while (  k < n - 15  &  tool[ k - 1 ] - tool[ k ] > tol  & nrow(mat) > 0 ) {
      ...
      if (ncores <= 1) {
        for ( i in 1:pn ) {
          ma <- survival::survreg( target ~., data = dataset[, c(sela, mat[i, 1]) ], weights = wei, dist = "loglogistic" )
          la <- logLik(ma)
        ...
        registerDoParallel(cl)
        mod <- foreach( i = 1:pn, .combine = rbind, .export = "survreg", .packages = "survival") %dopar% {
          ww <- survival::survreg( target ~., data = dataset[, c(sela, mat[i, 1]) ], weights = wei, dist = "loglogistic" )
          la <- logLik(ww)
          return( - 2 * as.numeric( la ) + attr(la, "df") * con )
        }
        stopCluster(cl)
  ...
  if ( d >= 1 ) {
    final <- survival::survreg( target ~., data = dataset[, sela, drop = FALSE], weights = wei, dist = "loglogistic" )
  }
