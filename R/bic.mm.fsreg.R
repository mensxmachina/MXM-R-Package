bic.mm.fsreg <- function( target, dataset, wei = NULL, tol = 0, ncores = 1) {
  ...
  dataset <- as.data.frame(dataset)
   ...
   #########
    ...
    if ( d >= 1 ) {
      final <- MASS::rlm( target ~., data = dataset[, sela], maxit = 2000, method = "MM")
      info <- info[1:d, , drop = FALSE]
      if ( d == 1 )  info <- matrix(info, nrow = 1)
      colnames(info) <- c( "variables", "BIC" )
      rownames(info) <- info[, 1]
    }  
    list( runtime = duration, mat = t(mat), info = info, ci_test = "testIndMMReg", final = final)
}
