ebic.llr.bsreg <- function(target, dataset, wei = NULL, gam = NULL) {
  ...
  if ( p > n ) {
    ...
    if ( M == 0 ) {
      ...
      ########### 
      for (j in 1:p) {
        mod <- survival::survreg( target ~.,  data = dataset[, -j, drop = FALSE], weights = wei, dist = "loglogistic" )
      ...
      if ( bic0 - mat[sel, 2] < 0  ) {
        ...
        if ( tool[2] != 0 ) {
          ...
          while ( tool[i - 1] - tool[i ] > 0  &  NCOL(dat) > 0 )  {   
            ...
            if ( M == 1 ) {
              mod <- survival::survreg(target ~ 1, weights = wei, dist = "loglogistic" )
              bic <-  - 2 * logLik(mod) + (length(mod$coefficients) + 1) * logn 
              tool[i] <- bic
              if (bic0 - bic < 0 ) {
                runtime <- proc.time() - tic
