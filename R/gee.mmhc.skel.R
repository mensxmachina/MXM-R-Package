gee.mmhc.skel <- function(dataset, group, max_k = 3, threshold = 0.05, test = "testIndGEEReg", type = "MMPC.gee", se = "jack", 
  ...
  if ( type == "MMPC.gee" ) {
    ...
    if (nc <= 1  ||  is.null(nc) ) {
      ...
      for (i in 1:n) {
        ...
        a <- MMPC.gee(i, group = group, dataset = dataset, max_k = max_k, threshold = threshold, test = test, ini = ini, hash = hash, se = se)
      ...
      mod <- foreach(i = 1:n, .combine = rbind, .export = c("MMPC.gee") ) %dopar% {
        sel <- numeric(n)
        if ( !is.null(ini.pvalue) )   ini$pvalue <- ini.pvalue[i, ]
        a <- MMPC.gee(i, group = group, dataset = dataset, max_k = max_k, threshold = threshold, test = test, ini = ini, hash = hash, se = se)
    ...
    if (nc <= 1  ||  is.null(nc) ) {
      ...
      for (i in 1:n) {
        if ( !is.null(ini.pvalue) )   ini$pvalue <- ini.pvalue[i, ]
        a <- SES.gee(i, group = group, dataset = dataset, max_k = max_k, threshold = threshold, test = test, ini = ini, hash = hash, se = se)
        ini.pval[i, ] <- a@univ$pvalue
        poies <- a@signatures
        ms[i] <- dim(poies)[1]
        sel <- unique(poies)
      ...
      registerDoParallel(cl)
      if ( !is.null(ini.pvalue) )   ini$stat <- rnorm(n)
      mod <- foreach(i = 1:n, .combine = rbind, .export = c("SES.gee") ) %dopar% {
