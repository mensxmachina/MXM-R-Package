iamb.normlogbs <- function(target, dataset, threshold = 0.05, wei = NULL) {
  ...
  if ( p > n ) {
    ...
    a1 <- internaliamb.normlogbs( target = target, dataset = dataset, threshold = threshold, wei = wei, p = p ) 
    ind <- 1:p
    a2 <- list()
    poies <- a1$mat[, 1]
    if ( length(poies) > 0 ) {
      ...
      a2 <- internaliamb.normlogbs( target = target, dataset = dat, threshold = threshold, wei = wei, p = length(ind) ) 
      poies <- a2$mat[, 1]
      ind[-poies] <- 0
      ind <- ind[ind > 0]
