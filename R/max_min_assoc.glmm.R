max_min_assoc.glmm = function(target, reps, group, dataset, test, wei, threshold, max_k, selectedVars, pvalues, stats, remainingVars, univariateModels, selectedVarsOrder, hash, stat_hash, pvalue_hash, slopes)
{
  selected_var = -1;
  selected_pvalue = 2;
  selected_stat = 0;
  ...
  for (cvar in varsToIterate) {
    mma_res = min_assoc.glmm(target, reps, group, dataset, test, max_k, cvar, wei, selectedVars, pvalues, stats, univariateModels, selectedVarsOrder, hash, stat_hash, pvalue_hash, slopes = slopes);
