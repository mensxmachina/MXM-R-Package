quasipois.fsreg <- function(target, dataset, ini = NULL, threshold = 0.05, wei = NULL, tol = 2, ncores = 1) {
  
  if ( !is.null(ini) ) {
    result <- quasipois.fsreg_2(target, dataset, iniset = ini, wei = wei, threshold = threshold, tol = tol, ncores = ncores) 
    ...
    runtime <- proc.time()
    ini <- glm( target ~ 1, family = quasipoisson(link = log), weights = wei, y = FALSE, model = FALSE )$deviance
    
    if (ncores <= 1) {
      
      for (i in 1:p) {
        mi <- glm( target ~ dataset[, i], family = quasipoisson(link = log), weights = wei, y = FALSE, model = FALSE )
        devi[i] <- mi$deviance
      ...
      mod <- foreach( i = 1:p, .combine = rbind) %dopar% {
        ww <- glm( target ~ dataset[, i], family = quasipoisson(link = log), weights = wei, y = FALSE, model = FALSE )
    ...
    if ( mat[sel, 2] < threshold ) {
      ...
      mi <- glm( target ~ dataset[, sel], family = quasipoisson(link = log), weights = wei, y = FALSE, model = FALSE )
      tool[1] <- BIC( mi )
      moda[[ 1 ]] <- mi
    }  else  {
      info <- info  
    ...
    if ( info[k, 2] < threshold  &  nrow(mat) > 0 ) {
      ...
      if ( ncores <= 1 ) {
        devi <- dof <- numeric(pn)
        for ( i in 1:pn ) {
          ww <- glm( target ~., data = dataset[, c(sela, mat[i, 1]) ], family = quasipoisson(link = log), weights = wei, y = FALSE, model = FALSE )
          devi[i] <- ww$deviance
      ...
      if ( mat[ina, 2] < threshold ) {
        ma <- glm( target ~ dataset[, sela] + dataset[, sel], family = quasipoisson(link = log), weights = wei, y = FALSE, model = FALSE )
        tool[k] <- BIC( ma )
    ...
    if ( nrow(info) > 1  &  nrow(mat) > 0 ) {
      ...
      while ( ( info[k, 2] < threshold ) &  ( k < n ) & ( tool[ k - 1 ] - tool[ k ] > tol ) & ( nrow(mat) > 0 ) )  {
        ...
        if (ncores <= 1) {  
          devi = dof = numeric(pn) 
          for ( i in 1:pn ) {
            ma <- glm( target ~., data = dataset[, c(sela, mat[i, 1] ) ], family = quasipoisson(link = log), weights = wei, y = FALSE, model = FALSE )
            devi[i] <- ma$deviance
        ...
        if ( mat[ina, 2] < threshold ) {
          ma <- glm( target ~., data = as.data.frame( dataset[, c(sela, sel) ] ), family = quasipoisson(link = log), weights = wei, y = FALSE, model = FALSE )
          tool[k] <- BIC( ma )
          if ( tool[ k - 1 ] - tool[ k  ] <= tol ) {
            info <- rbind(info, c( 1e300, 0, 0 ) )
    ...
    if ( d >= 1 ) {
      final <- glm( target ~., data = dataset[, sela, drop = FALSE], family = quasipoisson(link = log), weights = wei, y = FALSE, model = FALSE )
    ...
    result = list( runtime = runtime, mat = mat, info = info, ci_test = "testIndQPois", final = final ) 
