wr.fsreg <- function(target, dataset, ini = NULL, threshold = 0.05, wei = NULL, tol = 2, ncores = 1) {
  
  if ( !is.null(ini) ) {
    result <- wr.fsreg_2(target, dataset, iniset = ini, wei = wei, threshold = threshold, tol = tol, ncores = ncores) 
    ...
    if (ncores <= 1) {
      ...
      if ( mat[sel, 2] < threshold ) {
        ...
        mi <- survival::survreg( target ~ dataset[, sel], data = as.data.frame(dataset), weights = wei )
      ...
      if ( info[k, 2] < threshold  &  nrow(mat) > 0 ) {
        ...
        if ( ncores <= 1 ) {
          ...
          mod <- foreach( i = 1:pn, .combine = rbind, .export = "survival", .packages = "survreg") %dopar% {
            ww <- survival::survreg( target ~., data = as.data.frame( dataset[, c(sela, mat[i, 1]) ] ), weights = wei )
        ...
        if ( mat[ina, 2] < threshold ) {
            ma <- survival::survreg( target ~ dataset[, sela] + dataset[, sel], weights = wei )
      ...
      if ( info[k, 2] < threshold  &  nrow(mat) > 0 ) {
        ...
        while ( ( info[k, 2] < threshold ) &  ( k < n ) & ( tool[ k - 1 ] - tool[ k ] > tol ) & ( nrow(mat) > 0 ) )  {
          ...
          if (ncores <= 1) {  
            devi = dof = numeric(pn) 
            for ( i in 1:pn ) {
              ma <- survival::survreg( target ~., data = as.data.frame( dataset[, c(sela, mat[i, 1] ) ] ), weights = wei)
              devi[i] <- 2 * logLik(ma)
          ...
          if ( mat[ina, 2] < threshold ) {
              ma <- survival::survreg( target ~., data = as.data.frame( dataset[, c(sela, sel) ] ), weights = wei )
              tool[k] <- - 2 * logLik(ma) + ( length( ma$coefficients) + 1 ) * con	
    ...
    if ( d >= 1 ) {
      final <- survival::survreg( target ~., data = as.data.frame( dataset[, sela] ), weights = wei )
